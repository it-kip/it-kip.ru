...
class model_kolap extends CI_Model{
    ...
    function create_dir($Name, $fields = false, $tree = false){
        ...
        if(is_array($fields))
            foreach ($fields as $value){
                $value = trim($value);
                if($value != "")
                    $str[] = "{$Name}_{$value} VARCHAR(64)".(($this->db->dbdriver == 'mysql' && $value != $value)? " COMMENT '{\"n\":\"{$value}\"}'": "");
            }
        
        if($this->db->dbdriver == 'mysql'){
            $query = "CREATE TABLE `{$Name}` (
                `{$Name}_id` INT(11) NOT NULL AUTO_INCREMENT,
                `{$Name}_name` VARCHAR(32) NOT NULL COMMENT '{type: name}',
                ".implode(", ", $str).((!empty($str))? ", ": "")."
                ".($tree? $Name.'_parent INT(11), ': '')."
                PRIMARY KEY  (`{$Name}_id`),
                ".($tree
                    ? "UNIQUE KEY `{$Name}_un` (`{$Name}_name`,`{$Name}_parent`),
                    KEY `FK_{$Name}` (`{$Name}_parent`),
                    CONSTRAINT `FK_{$Name}` FOREIGN KEY (`{$Name}_parent`) REFERENCES `{$Name}` (`{$Name}_id`) ON DELETE RESTRICT ON UPDATE CASCADE "
                    : "UNIQUE KEY `{$Name}_un` (`{$Name}_name`)")."
        ...
        if($Name != "" && isset($One)){
            ...
            if($this->db->dbdriver == 'mysql'){
                foreach($Out as $key => &$o)
                    if($o != "")
                        $o = " `".$Name."_".$this->translit($o)."` INT COMMENT '{$o}'";
                    else
                        unset($Out[$key]);
                ...
                $query = "CREATE TABLE `$Name` (
                            ...
                            CONSTRAINT `".$Name."_pkey` PRIMARY KEY(`".$Name."_id`),
                            
                            UNIQUE KEY `unq_$Name` (`".$Name."_".implode("`, `".$Name."_", $One)."`),
                            ".implode(", ", $fk)."
                            ) ENGINE=INNODB DEFAULT CHARSET=utf8 COMMENT='".json_encode(array('name' => $Name))."'";
$query.= ";".implode("; ", $constr).";";
                $this->db->query($query);

                // многий ко многим
                if(!empty($Many))
                    foreach($Many as $dir){
                        $query = "CREATE TABLE {$Name}_{$dir} (
                                    `{$Name}_{$dir}_id` INT NOT NULL AUTO_INCREMENT,
                                    {$Name}_{$dir} INT NOT NULL,
                                    {$dir}_{$Name} INT NOT NULL,
                                    PRIMARY KEY (`{$Name}_{$dir}_id`),
                                    UNIQUE KEY `unq_{$Name}_{$dir}` ({$Name}_{$dir}, {$dir}_{$Name}),
                                    KEY `FK_{$Name}_{$dir}` (`".$Name."_".$dir."`),
                                    KEY `FK_{$dir}_{$Name}` (`".$dir."_".$Name."`),
                                    CONSTRAINT `FK_{$Name}_{$dir}` FOREIGN KEY (`".$Name."_".$dir."`) REFERENCES `".$CubeName."` (`id_$CubeName`) ON DELETE CASCADE ON UPDATE CASCADE,
                                    CONSTRAINT `FK_{$dir}_{$Name}` FOREIGN KEY (`".$dir."_".$Name."`) REFERENCES `".$dir."` (`{$this->keyTable($dir)}`) ON DELETE RESTRICT ON UPDATE CASCADE
            ...
            elseif($this->db->dbdriver == 'postgre'){ // нет многий ко многим
                ...
                foreach($Out as $key => &$o)
                    if($o != "")
                        $o = $Name."_".$this->translit($o)." INTEGER";
                    else
                        unset($Out[$key]);
    ...
    function set_slices($cube, $slice, $val){
        $arr = array();
        foreach($val as $key => $v) $arr[] = "`{$key}` = ".$v;
        
        $q = "insert into `{$cube}` (`". implode("`, `", array_keys($slice)) ."`, `". implode("`, `", array_keys($val)) ."`) ";
        $q.= "values (". implode(", ", array_values($slice)) .", ". implode(", ", array_values($val)) .") ";
        $q.= "ON DUPLICATE KEY UPDATE ".  implode(", ", $arr);
        
        return $this->db->query($q);
    }
    
    function set_rec($tbl, $fr){
        $res = $this->db->get_where($tbl, $fr);
        if(!$res->num_rows()){
            $this->db->insert($tbl, $fr);
            $res = $this->db->get_where($tbl, $fr);
