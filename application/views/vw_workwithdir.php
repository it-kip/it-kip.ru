...
<h2><?= isset($table['info']['n'])? $table['info']['n']: $table['name']; ?></h2>
...
<script>
        ...
        $("#new").dialog({
            ...
            buttons: {
                'Сохранить': function(){
                    ...
                    $.post('/kolap/addrec/<?= $table['name']; ?>', post, function(data){
    ...
    function info(el){
        ...
        $.post("/kolap/dir_vals/<?= $table['name']; ?>/" + id, null, function(data){
    ...
    function expand(el, add){
        ...
        if(uls.length > 0){
            ...
            $.post('/kolap/tree_children/<?= $table['name']; ?>/' + id, null, function(data){
                if(data.length > 0){
        ...
        $("#tree li").draggable({
            ...
            stop: function(){
                if(d != undefined){
                    ...
                    $.post('/kolap/rechild/<?= $table['name']; ?>', {parent: $(d).find('p')[0].getAttribute("value"), child: $(this).find('p')[0].getAttribute("value")});
                    d = undefined;
                }
            }
        });
